...
echo
echo "Prepare data for Opentsdb...."
...
bin/bulk_data_gen -seed 123 -format opentsdb -sampling-interval 10s -scale-var 100 -use-case devops -timestamp-start "2018-01-01T00:00:00Z" -timestamp-end "2018-01-02T00:00:00Z" > /mnt/opentsdb.dat
...
bin/bulk_data_gen -seed 123 -format tdengine -sampling-interval 10s -tdschema-file config/TDengineSchema.toml -scale-var 100 -use-case devops -timestamp-start "2018-01-01T00:00:00Z" -timestamp-end "2018-01-02T00:00:00Z"  > /mnt/tdengine.dat
...
TDENGINERES=`cat /mnt/tdengine.dat |bin/bulk_load_tdengine --url 127.0.0.1:0 --batch-size 5000   -do-load -report-tags n1 -workers 16 -fileout=false| grep loaded`
...
sleep 10
echo
echo -e "Start test Opentsdb, result in ${GREEN}Green line${NC}"

#curl 'http://localhost:8086/query?q=drop%20database%20benchmark_db' -X POST

OPENTSDBS=`cat /mnt/opentsdb.dat  |bin/bulk_load_opentsdb --batch-size=5000 --workers=16 --urls="http://127.0.0.1:4242" | grep loaded`


echo
echo -e "${GREEN}Opentsdb writing result:${NC}"
echo -e "${GREEN}$OPENTSDBS${NC}"

TMP=`echo $OPENTSDBS|awk '{print($5)}'`
IFWTM=`echo ${TMP%s*}`
